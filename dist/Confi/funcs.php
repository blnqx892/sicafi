<?php
	...
	function activarUsuario($id)
	{
		...
		$stmt = $mysqli->prepare("UPDATE suaurios SET activacion=1 WHERE id = ?");
	...
	function login($usuario, $password)
	{
		...
		$stmt = $mysqli->prepare("SELECT id, id_tipo, contrasena FROM usuarios WHERE usuario = ? || correo = ? LIMIT 1");
	...
	function cambiaPassword($password, $user_id, $token){
		...
		$stmt = $mysqli->prepare("UPDATE usuarios SET contrasena = ?, token_password='', password_request=0 WHERE id = ? AND token_password = ?");
